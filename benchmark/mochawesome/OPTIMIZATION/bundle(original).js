...
+function ($, Chart, _) {
    ...
    Mochawesome.prototype._onFilterClick = function (e) {
        ...
        var filter = $el.data('filter'), $btns = $('[data-filter=' + filter + ']'), filterIndex = this.activeFilters.indexOf(filter), filterIsActive = filterIndex !== -1;
    ...
    Mochawesome.prototype.updateFilteredTests = function () {
        var activeFiltersExist = this.activeFilters.length > 0, filterClassesToAdd = this._createFilterClasses('filter-'), testClassesToFilter = this._createFilterClasses('.');
        this.$details.removeClass(this.filterClasses).toggleClass('filters-active', activeFiltersExist);
        ...
        if (activeFiltersExist) {
            for (var i = this.$suites.length - 1; i >= 0; i--) {
                var $suite = this.$suites.eq(i), hasVisibleTests = $suite.find('.test').filter(testClassesToFilter.join()).length > 0;
                if (hasVisibleTests) {
    ...
    Mochawesome.prototype.makeSuiteCharts = function () {
        ...
        for (var i = 0; i < this.$suiteCharts.length; i++) {
            var $chart = this.$suiteCharts.eq(i), ctx = $chart[0].getContext('2d'), data = $chart.data(), chartData = [
                    {
